...
with indicadores:
        ...
        pipeline = [
            {
                "$addFields": {
                    "date": {
                        "$cond": [
                            {"$eq": [{"$type": "$Data da próxima realização"}, "string"]},
                            {"$toDate": "$Data da próxima realização"},
    ...
    with col1:
        st.metric('Total de testes para realizar', total_to_do)

    with col2:
        st.metric('Indicador de Realização Total', f'{indicador_realizacao:.2f}%'.replace('.', ','))

    with col3:
        st.metric('Indicador de Arquivamento Total', f'{indicador_arquivamento:.2f}%'.replace('.', ','))
    ...
    with col4:
        ...
        st.button('Atualizar dados', type='primary', on_click=refresh_data)
...
with arquivamento:
    ...
    st.markdown(
        ...
        **Observação:** Foi definido o período de um mês para realização do relatório.<br>
        O status de arquivamento é atualizado ao clicar na caixa de seleção. Arquive um teste por vez.
        </span>
        """,
